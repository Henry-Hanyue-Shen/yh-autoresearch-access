...
```powershell
py skills/yh-autoresearch-access/scripts/install_yh_autoresearch.py \
  --base-url $env:YH_AUTORESEARCH_ACCESS_URL \
  --agent codex
```
...
Open the exact HTTPS activation URL supplied by YH, enter the internal-beta code, and download the verified ZIP. Install or attach the `yh-autoresearch` directory using that agent's supported skill/file flow. The bundle is self-contained after download.
